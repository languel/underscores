...
This note records the contracts behind Underscores's non-destructive **Brush → Stack** implementation. The old **Mods & FX** panel id and `/mods` alias remain for compatibility. Preserve these invariants when adding brushes, filters, transforms, or history behavior.
...
- Mods & FX uses Underscores's shared panel shell. Its width, floating size, placement, visibility, and dock-tab state persist independently from other panels. The panel icon handles activation, thresholded drag-to-float/dock, and a right-click placement menu; a plain click never detaches the panel.
