...
Versioned sketch metadata distinguishes three uses without replacing the existing internal exchange kinds:

- `project`: a complete sketch and authored Underscores state.
- `fragment`: selected portable objects and their required authored dependencies.
- `help`: an explanatory sketch that may also link to snippets and walkthroughs.
...
The Info panel contains the first bundled help catalog. Catalog entries are searchable and may offer **Insert Sketch**, **Insert Snippet**, or **Start Walkthrough**. Insert actions reuse registered commands and selection import instead of directly mutating the scene. The initial entries cover onboarding, p5, GLSL, and audio/physics; the same model can later support a comprehensive bundled or user-installed help library.
