...
**Live pose** is a separate solver-driven manipulation mode. Enable it in Physics World, pause the simulation (or set its time scale near zero), then drag either an authored body or an axle/weld pivot. The grab is full-strength at pointer speed while the solver performs bounded zero-gravity constraint iterations, so attached bodies propagate like an IK rig without advancing the public physics/transport clock. With Live pose off, hold `Cmd` while dragging a body for that one temporary full-strength grab. At timeline zero, releasing the grab promotes the solved result to the authored physics reset pose, so rewind returns to it. At any other time the result remains a temporary runtime pose; use **Apply pose** when that is the intended new reset baseline.
