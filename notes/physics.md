...
3. Use the compact canvas Physics toolbar for fast authoring. **Fixate** and **Axle** are authored pivot objects: choose the tool, then click a separate visible canvas object that marks the pivot. Its centre is tested against the physics bodies beneath it. One overlapping body is attached to World; two overlapping bodies are joined at that point. The pivot owns its full constraint configuration at `object.customData.physics` and remains selectable, copyable, and deletable like any other canvas object. **Axle** rotates freely through 360° by default; opt into angular limits only when needed. Legacy graph-only Spring tools still use two attachment clicks while their canvas-object authoring is redesigned.
