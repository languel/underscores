...
3. Use the compact floating Physics toolbar for fast authoring. **Weld** and **Axle** turn selected canvas objects into authored pivot constraints. The pivot centre is tested against the physics bodies beneath it: one overlapping body attaches to World; two overlapping bodies join at that point. The pivot owns its full constraint configuration at `object.customData.physics` and remains selectable, copyable, and deletable like any other canvas object. **Axle** rotates freely through 360° by default; opt into angular limits only when needed. The toolbar is draggable by its atom icon, can be minimized with Shift-double-click, closed with Option-double-click or its context menu, and reopened with `/physicstoolbar`.
4. Edit a named first-class constraint in the Physics panel's **Constraints** section or in the pivot object's Physics role in Properties. It exposes endpoints, enable/collide-connected state, and the relevant spring or hinge settings. Pin/Revolute/Weld remain compatible vocabulary for existing scenes; the persisted legacy `fixate` kind is displayed as **Weld**.
