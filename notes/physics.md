...
Use **Physics → Collision layers** to name the world layers and enable or disable each layer pair. The selected-body editor and Properties expose memberships as checkboxes; a multi-selection applies the chosen membership set to every selected physics body. This makes articulated rigs straightforward: put all links on `Rig`, disable `Rig × Rig` to suppress self-collisions, and leave `Rig × World` enabled. One body can belong to more than one layer when it needs to interact with several groups.
...
Rope cost is determined by its sampled solver links, not the number of points in the source drawing. Sampling is uniform by path arc length and capped at 96 links; a 1,200 px dense stroke with a 24 px link length creates about 58 links instead of hundreds of bodies and joints. Generated links share a private runtime collision group, which disables all rope-internal contact pairs and avoids a quadratic contact workload. Authored bodies and walls include that private bit in their collision masks, so ropes remain physically useful against the normal scene while self-collision is deferred to a future quality mode.
