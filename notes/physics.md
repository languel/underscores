...
4. A **Rope** is not a special Rapier scene entity. It samples the authored line, arrow, freehand path, Bézier curve, or spline into short capsule links joined by revolute joints in the worker. The generated links never become canvas objects or serialize individually; the single authored path redraws from their solved endpoints. Use **Link length** to trade curve fidelity for solver cost, and **Thickness** for the physical rope diameter without changing the source stroke. Adjacent links do not collide by default for a stable rope; enable **Collide while connected** only when that interaction is intentional. Run `/make rope` after selecting a compatible path.
