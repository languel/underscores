...
Version 3 adds named **collision layers**. `world.collisionLayers.layers` is a stable stack of up to 16 named layers and `world.collisionLayers.matrix` is its symmetric, editable contact table. A body's `collisionLayers` array selects its memberships; the worker derives the Rapier group/mask pair when it loads the graph. New worlds start with one **Default** layer and all contact pairs enabled, preserving ordinary all-to-all collisions. Bodies from earlier scenes deliberately keep their raw `collisionGroup` and `collisionMask` until they are assigned to the named stack, so migration never silently changes an existing simulation.
