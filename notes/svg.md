...
Underscore bindings all patch that same source; the editor does not maintain a competing flattened
scene graph.

Version 2 of `customData.underscoreSvg` is:
...
`data-underscore-id` attributes to addressable nodes and, when needed, an embedded registry:

```xml
<metadata data-underscore="v1">
  {"version":1,"nodes":{"svg-node-…":{"iannix":{"role":"curve"}}}}
</metadata>
```

Authored `id` attributes are never replaced. The embedded registry is canonical for Underscore
...
policy, and embedded Underscore data. Matched stylesheet declarations are patched in their existing
...
Underscore transport is the default master clock. SMIL is sought with
...
- Underscore metadata automation lanes
...
one history change. Neutral foreground marks become `currentColor`, so they follow Underscore's
...
Underscore, Transparent, Mono, VS Code, and Teaching skins while retaining immediate valid-source
