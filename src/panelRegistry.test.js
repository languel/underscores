...
test("dock registry keeps the requested primary right and bottom tab order", () => {
  ...
  assert.deepEqual(rightDockOrder, [
    "grid", "outliner", "properties", "iannix", "mods", "synth", "video-input", "media-input", "holistic", "mapping", "script", "chat", "history", "console", "settings", "mixer", "info",
  ]);
...
test("panel lookup and slash matching share one registry", () => {
  ...
  assert.equal(getDraweratorPanel("video-input")?.slash, "/video-input");
