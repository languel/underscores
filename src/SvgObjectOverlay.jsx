...
const SvgShadowDocument = ({ source, color, clock, time, interactive, onSelect, onEditNode, onConstructPath }) => {
  ...
  useEffect(() => {
    ...
    style.textContent = `:host,.underscores-svg-shadow-content{display:block;width:100%;height:100%}:host{color:${color || "currentColor"}}svg{display:block;width:100%;height:100%;overflow:visible}`;
    const content = document.createElement("div");
    content.className = "underscores-svg-shadow-content";
    content.innerHTML = sanitizeSvgForInertRender(source);
    shadow.replaceChildren(style, content);

    const nodeFromEvent = event => {
      const target = event.composedPath?.().find(item => item instanceof Element && item.hasAttribute?.("data-underscores-render-index"));
      const nodeIndex = Number(target?.getAttribute?.("data-underscores-render-index"));
  ...
  return <div ref={hostRef} className="underscores-svg-shadow-host" />;
};
...
export default function SvgObjectOverlay({ elements, appState, time = 0, onSelect, onEditPath, onEditNode, onConstructPath }) {
  ...
  return (
    <div className="underscores-svg-object-overlay" aria-hidden="true">
      {objects.map((element, layerIndex) => {
        ...
        return (
          <div
            key={element.id}
            data-underscores-svg-element-id={element.id}
            className={`underscores-svg-object-frame ${interactive ? "underscores-svg-object-frame-interactive" : ""}`}
            style={{
