...
const DRAWERATOR_SVG_METADATA_ID = "drawerator-editable-elements";
const DRAWERATOR_SVG_METADATA_VERSION = 1;
...
const resolveStyle = (inherited, attributes) => {
  ...
  return {
    // Leave absent strokes unresolved so Drawerator can use the active theme's
...
// metadata when Drawerator copies a selection, so a later Drawerator paste can
...
export const attachDraweratorSvgMetadata = (input, elements) => {
  const svg = cleanSvgMarkup(input);
  const source = Array.isArray(elements) ? elements : [];
  if (!svg || source.length === 0) return svg;
  const payload = encodeURIComponent(JSON.stringify({
    version: DRAWERATOR_SVG_METADATA_VERSION,
    elements: source,
  }));
  return svg.replace(/<\/svg\s*>\s*$/i, `<metadata id="${DRAWERATOR_SVG_METADATA_ID}" data-encoding="uri">${payload}</metadata></svg>`);
};

export const extractDraweratorSvgMetadata = input => {
  ...
  while ((match = metadata.exec(svg))) {
    ...
    if (attributes.id !== DRAWERATOR_SVG_METADATA_ID || attributes["data-encoding"] !== "uri") continue;
    try {
      const payload = JSON.parse(decodeURIComponent(match[2].trim()));
      if (payload?.version !== DRAWERATOR_SVG_METADATA_VERSION || !Array.isArray(payload.elements)) return null;
