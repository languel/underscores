...
test("Strudel transport maps Underscore BPM to four-beat cycles", () => {
...
test("Strudel Linked patterns use the Underscore score phase", () => {
  const runtime = new StrudelRuntimeManager();
...
test("Strudel linked clock CPS commands update the shared Underscore tempo", () => {
  const runtime = new StrudelRuntimeManager();
  const tempos = [];
...
test("Strudel runtime compiles REPL-style anonymous pattern directives", async () => {
  ...
  try {
    ...
    assert.ok(pattern.queryArc(0, 1)[0].context.tags.includes("underscore:node-a"));
  } finally {
