...
const PROPERTIES_PINS_STORAGE_KEY = "underscores_properties_pins_v1";
...
const isObjectReferencePath = path => pathKey(path) === "customData.underscoresMediaStream.canvas.elementId";

const defaultPinnedPathsFor = element => {
  if (element?.type === "frame") return [
    ["customData", "underscoresFrame", "label"],
    ["customData", "underscoresFrame", "showLabel"],
  ];
  if (isMediaStreamElement(element)) {
    ...
    if (stream.kind === MEDIA_STREAM_KINDS.PREVIEW) return [
      ["customData", "underscoresMediaStream", "sourceId"],
      ["customData", "underscoresMediaStream", "enabled"],
      ["customData", "underscoresMediaStream", "mirror"],
    ];
  }
  ...
  return (
    <details className="properties-group properties-embed-group" open>
      ...
      <div className="properties-children">
        ...
        {matches("interaction") && <div className="properties-row editable"><span>interact</span><input type="checkbox" checked={policy.allowInteraction} onChange={event => onChange(["customData", "underscoresEmbed", "allowInteraction"], event.target.checked)} /></div>}
        {matches("crop") && [["cropTop", "crop top"], ["cropRight", "crop right"], ["cropBottom", "crop bottom"], ["cropLeft", "crop left"]].map(([key, label]) => <div className="properties-row editable" key={key}><span>{label} px</span><NumericInput min="0" step="1" value={policy[key]} defaultValue={0} onCommit={value => onChange(["customData", "underscoresEmbed", key], value)} /></div>)}
        {matches("css") && <div className="properties-row properties-embed-css editable"><span>inject CSS</span><textarea value={policy.css} onChange={event => onChange(["customData", "underscoresEmbed", "css"], event.target.value)} placeholder="body { margin: 0; }" /></div>}
        {!query?.needle && <p className="properties-embed-note">HTTP(S) only. Embeds load in every view by default; choose “Presentation only” or disable the instance when it should stay dormant. Interact passes mouse input to the page; turn it off to select or transform the embed. Crop hides fixed page chrome. CSS is injected only into same-origin embeds—browser security prevents it for external sites such as p5.js.</p>}
      </div>
    </details>
  );
};
...
const P5FrameControls = ({ element, query, onChange }) => {
  ...
  const update = patch => onChange(["customData", "underscoresP5"], normalizeP5Frame({ ...frame, ...patch }));
  return (
    <details className="properties-group properties-p5-group" open>
      ...
      <div className="properties-children">
        ...
        {!query?.needle && <p className="properties-p5-note">Trusted local code: this sketch runs directly in Underscores with full page access. Use only scripts you trust. Bundled p5 is included with Underscores; CDN mode loads a compatible runtime from the URL above.</p>}
      </div>
    </details>
  );
};
...
const normalizeFramePresentation = element => {
  const value = element?.customData?.underscoresFrame || {};
  return {
...
const FrameControls = ({ element, query, onChange }) => {
  ...
  const hasFramePresentation = Boolean(element?.customData?.underscoresFrame);
  // Native Excalidraw frames predate the Underscores presentation data. Adopt
  // a selected legacy frame once, with the new label-hidden default.
  useEffect(() => {
    if (!isFrame || hasFramePresentation) return;
    onChange(["customData", "underscoresFrame"], presentation);
  ...
  const update = patch => onChange(["customData", "underscoresFrame"], { ...presentation, ...patch });
  return (
...
const MediaPreviewControls = ({ element, query, onChange, mediaSources = [], onFocusMediaSource }) => {
  ...
  const update = patch => onChange(["customData", "underscoresMediaStream"], patchMediaStreamConfig(stream, patch));
  return (
...
const PinnedPropertyControls = ({ element, query, onChange, mediaSources, pinnedPaths, pinnedKeys, onTogglePin, onPickObjectReference }) => {
  ...
  const isPreview = isMediaStreamElement(element)
    && normalizeMediaStreamConfig(element.customData?.underscoresMediaStream).kind === MEDIA_STREAM_KINDS.PREVIEW;
  const isFrame = element?.type === "frame";
  const paths = [...defaults, ...pinnedPaths.map(key => key.split("."))]
    .filter(path => !isPreview || !pathKey(path).startsWith("customData.underscoresMediaStream."))
    .filter(path => !isFrame || !pathKey(path).startsWith("customData.underscoresFrame."))
...
const SvgObjectControls = ({
  ...
  const selectedSubpath = Number.isInteger(selectedSubpathIndex)
    ...
    : null;
  const selectedNodeData = selectedNode?.underscoresId
    ? svg.metadataMirror?.nodes?.[selectedNode.underscoresId] || {}
    : {};
  ...
  return (
    <details className="properties-group properties-svg-group" open>
      ...
      <div className="properties-children">
        ...
        {matches("animation") && <details className="properties-svg-editor-section" open={timingGraph.lanes.length > 0}>
          ...
          <div className="properties-children">
            <div className="properties-row editable">
              ...
              >
                <option value="transport">Underscores transport</option>
            ...
            {analysis.hasScript && <>
              ...
              <p className="properties-p5-note">{svg.runtime.trustedScripts
                ? "Scripts run in a sandboxed, cross-origin iframe. They cannot access Underscores or mutate canonical source; only the limited cue/log/MIDI bridge is exposed."
        ...
        {!query?.needle && <p className="properties-svg-note">Select a path or subpath for canvas editing. Score roles can live directly on SVG nodes; extraction creates a separate native Underscores spline. Canonical source remains editable from the SVG type in Script.</p>}
      </div>
    </details>
  );
};
...
const PropertiesPanel = memo(function PropertiesPanel({
  ...
  const svgCurveOptions = useMemo(() => (availableElements || []).flatMap(element => {
    ...
    return Object.entries(svg.metadataMirror?.nodes || {}).flatMap(([nodeId, data]) => {
      ...
      const node = nodes.find(candidate => candidate.underscoresId === nodeId);
      const ref = {
        kind: "svg-node",
