...
const NumberField = ({ label, value, onChange, defaultValue, ...inputProps }) => (
    ...
    <div className="grid-panel" role="form" aria-label="Global grid controls">
      <section className="grid-panel-group grid-panel-geometry" aria-label="Grid geometry">
        ...
        <div className="grid-panel-row">
          ...
          <NumberField label="Spacing X" aria-label="Global grid spacing X" min="1" step="1" defaultValue="100" value={grid.spacing.x} onChange={event => onUpdate({ spacing: { x: event.target.value } })} />
          <NumberField label="Spacing Y" aria-label="Global grid spacing Y" min="1" step="1" defaultValue="100" value={grid.spacing.y} onChange={event => onUpdate({ spacing: { y: event.target.value } })} />
          <NumberField label="Rotation" aria-label="Global grid rotation" step="1" defaultValue="0" value={Math.round(grid.transform.rotation * 180 / Math.PI * 100) / 100} onChange={event => onUpdate({ transform: { rotation: Number(event.target.value) * Math.PI / 180 } })} />
        </div>
        <div className="grid-panel-row">
          <NumberField label="Sub X" aria-label="Global grid X subdivisions" min="1" max="64" step="1" defaultValue="5" value={grid.spacing.subdivisionsX} onChange={event => onUpdate({ spacing: { subdivisionsX: event.target.value } })} />
          <NumberField label="Sub Y" aria-label="Global grid Y subdivisions" min="1" max="64" step="1" defaultValue="5" value={grid.spacing.subdivisionsY} onChange={event => onUpdate({ spacing: { subdivisionsY: event.target.value } })} />
          <NumberField label="Origin X" aria-label="Global grid origin X" step="1" defaultValue="0" value={grid.transform.origin[0]} onChange={event => onUpdate({ transform: { origin: [event.target.value, grid.transform.origin[1]] } })} />
          <NumberField label="Origin Y" aria-label="Global grid origin Y" step="1" defaultValue="0" value={grid.transform.origin[1]} onChange={event => onUpdate({ transform: { origin: [grid.transform.origin[0], event.target.value] } })} />
        </div>
      ...
      <section className="grid-panel-group grid-panel-snap" aria-label="Grid snapping">
        ...
        <div className="grid-panel-row">
          ...
          <NumberField label="Radius px" aria-label="Global grid magnetic threshold" min="1" max="64" step="1" defaultValue="8" value={grid.snap.thresholdPx} onChange={event => onUpdate({ snap: { thresholdPx: event.target.value } })} />
        </div>
      ...
      <section className="grid-panel-group grid-panel-time" aria-label="Grid time mapping">
        ...
        <div className="grid-panel-row">
          <NumberField label="Per cell" aria-label="Global grid time amount" min="0.000001" step="0.25" defaultValue="1" value={grid.time.amount} onChange={event => onUpdate({ time: { amount: event.target.value } })} />
