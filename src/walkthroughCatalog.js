...
export const TIMELINE_WALKTHROUGH = createWalkthrough({
  ...
  steps: [
    ...
    },
    {
      ...
      info: "Visual and document kinds default to Free. Strudel defaults to Linked and re-anchors its cycles to score time on play, rewind, loop, backward seek, and tempo change. A node's transport time is scoped to that node: __.transport.time, and its __.time shortcut, give global score time here, and the node's local clip time once it is playing from a Clip lanes track.",
      focusTarget: "canvas.selection",
      cues: [
    ...
    },
    {
      ...
      narration: "A clock decides *how* an object reads time. A **clip** decides *when* it participates at all.\n\nSelect the Timeline pulse node and run **Add clip at playhead**. A lane appears in Timeline's **Object lanes** mode: drag the clip to move it, drag an edge to trim it, Option-drag an edge to stretch its rate, and double-click it to loop.",
    ...
    {
      id: "clip-lanes",
      title: "7. The other kind of lane",
      narration: "Object lanes asks *when each object participates*, so its lanes come from the objects. **Clip lanes** asks *what happens when*, so its tracks are yours.\n\nSwitch with the two buttons at the left of the timeline. Add a track with `+`, rename it, reorder it, then drag a **History action** from History or an object from **Outliner** onto it.",
      info: "Clip timing is independent of the source object's layer, so a node's place in the drawing has nothing to do with which track its clip sits on. Livecode and media follow the clip's local playhead; history command, scene, and MIDI cues fire once when their clip starts, which is what makes this mode work as a cue sheet.",
      focusTarget: "panel.transport",
      cues: [
        { type: "command", commandId: "panel.open", args: { panelId: "history" }, at: 0.3 },
        { type: "command", commandId: "panel.open", args: { panelId: "transport" }, at: 0.9 },
      ],
      advance: { mode: "continue" },
      hint: "Switching to Clip lanes the first time creates three empty tracks to drop onto. Your mode choice is remembered locally.",
    },
    {
      id: "record",
