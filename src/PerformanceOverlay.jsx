...
export default function PerformanceOverlay({ placement = "floating", onPlacementChange, onClose }) {
  ...
  const snapshot = useSyncExternalStore(
    underscorePerformanceMonitor.subscribe,
    underscorePerformanceMonitor.getSnapshot,
    underscorePerformanceMonitor.getSnapshot,
  );
  useEffect(() => {
    underscorePerformanceMonitor.setEnabled(true);
    return () => underscorePerformanceMonitor.setEnabled(false);
  }, []);
  useEffect(() => {
  ...
  return (
    <aside className={`underscore-performance-overlay ${attached ? "console" : "floating"} ${expanded ? "expanded" : ""}`} aria-label="Performance monitor">
