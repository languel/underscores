...
export const validateAIBrushSource = source => {
  ...
  const forbiddenReference = text.match(FORBIDDEN_RUNTIME_REFERENCES)?.[0];
  if (forbiddenReference) {
    errors.push(`AI brush scripts may not use ${forbiddenReference}, browser, storage, network, clock, or module globals. Use points and Underscores globals only.`);
  }
  try {
...
export const SCRIPT_AUTHORING_GUIDES = Object.freeze({
  iannix: [
    "Underscores IanniX-compatible script contract:",
    ...
    "- Write readable source: put each statement after a semicolon on its own line, indent blocks with two spaces, and never compress an entire script into one line. Underscores will also format saved AI scripts.",
