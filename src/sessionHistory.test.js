...
test("adjacent UI input samples share one clip until another action intervenes", () => {
  const controller = new UnderscoresSessionController({ now: () => 0 });
  ...
  assert.equal(session.actions[0].duration, 0.12);
  assert.equal(session.actions[0].args.samples.length, 2);
  assert.equal(session.actions[0].args.samples[1].time, 100);
  ...
  assert.equal(session.actions[2].args.samples.length, 1);
  assert.equal(session.actions[3].args.pointerType, "touch");
});
