...
export default function MediaMappingPanel({
  ...
  const config = processor ? normalizeMediaStreamConfig(processor.customData.underscoresMediaStream) : null;
    ...
    <label className="media-stream-panel-field">
      ...
      <select value={activeId} onChange={event => setProcessorId(event.target.value)}>
        ...
        {processors.map(element => {
          const item = normalizeMediaStreamConfig(element.customData.underscoresMediaStream);
