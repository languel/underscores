...
// authored label wins; native Excalidraw objects receive a stable type/id label
// so a fresh canvas is navigable before the author has named anything.
export const getOutlinerElementLabel = element => {
  ...
  return isNativeExcalidrawElement(element) ? getDefaultExcalidrawLabel(element) : element?.id || "";
};
...
const OutlinerPanel = memo(function OutlinerPanel({
  ...
  const renderElement = (element, depth) => {
    ...
    return <div className="outliner-entry" key={element.id}>
      <div role="treeitem" aria-level={depth + 1} aria-expanded={isSvg ? expandedSvgIds.has(element.id) : undefined} aria-selected={Boolean(selectedElementIds[element.id])} className={`outliner-row ${selectedElementIds[element.id] ? "selected" : ""} ${draggingIds.includes(element.id) ? "dragging" : ""} ${dropPlacement ? `drop-${dropPlacement}` : ""}`} ref={node => node ? rowRefs.current.set(element.id, node) : rowRefs.current.delete(element.id)} draggable={editingId !== element.id} onDragStart={event => startDrag(event, element.id)} onDragEnd={clearDrag} onDragOver={event => {
        ...
        <div role="button" tabIndex={0} className={`outliner-object ${isSvg ? "has-children" : ""}`} style={{ "--outliner-depth": depth }} onClick={event => selectElement(element.id, event)} onKeyDown={event => {
        ...
        }} title={`${getElementTypeLabel(element)} · ${getOutlinerElementLabel(element)} · ${element.id}${isSvg ? " · double-click row to expand; double-click label to rename" : " · double-click to rename"}`}>
          ...
          {editingId === element.id ? <input ref={editingRef} className="outliner-label-input" value={editingValue} placeholder={getOutlinerElementLabel(element)} onPointerDown={event => event.stopPropagation()} onClick={event => event.stopPropagation()} onDoubleClick={event => event.stopPropagation()} onChange={event => setEditingValue(event.target.value)} onBlur={() => finishRename(element)} onKeyDown={event => { event.stopPropagation(); if (event.key === "Enter") { event.preventDefault(); finishRename(element); } if (event.key === "Escape") { event.preventDefault(); finishRename(element, false); } }} aria-label={`Rename ${getOutlinerElementLabel(element)}`} /> : <span className="outliner-label" onDoubleClick={event => { event.preventDefault(); event.stopPropagation(); beginRename(element); }}>{nameMode === "labels" ? getElementLabel(element) : element.id}</span>}
        </div>
  ...
  const renderNode = (node, depth = 0) => {
    ...
    if (["group", "score", "iannix-group"].includes(node.kind)) {
      ...
      const isPresentationHidden = memberIds.length > 0 && memberIds.every(id => visibleElements.find(element => element.id === id)?.customData?.presentationVisible === false);
      ...
      return <div className={`outliner-group outliner-${type}`} key={`${node.kind}-${key}`}>
        <div role="treeitem" aria-level={depth + 1} aria-expanded={expanded} aria-selected={isSelected} className={`outliner-group-row ${isSelected ? "selected" : ""} ${draggingIds.some(id => memberIds.includes(id)) ? "dragging" : ""} ${dropTarget?.groupKey === key ? "drop-inside" : ""}`} style={{ "--outliner-depth": depth }} draggable={node.kind === "group"} onDragStart={event => node.kind === "group" && startGroupDrag(event, node)} onDragEnd={clearDrag} onDragOver={event => {
          ...
          <button type="button" className={isPresentationHidden ? "outliner-toggle inactive" : "outliner-toggle"} onClick={event => onPresentationVisibilityChange(actionIdsFor(event, memberIds))} title={`${isPresentationHidden ? "Show" : "Hide"} group in presentation`} aria-label={`${isPresentationHidden ? "Show" : "Hide"} ${label} in presentation`} aria-pressed={!isPresentationHidden} {...infoProps("Group presentation visibility", "Include or exclude every object in this group in Presentation. Their canvas visibility is unchanged. Option-click applies the action to the current selection.")}><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 8h4l2-2h4l2 2h4v10H4Z"/><circle cx="12" cy="13" r="3"/></svg></button>
