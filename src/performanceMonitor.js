...
export const countPerformanceScene = (elements = [], selectedElementIds = {}) => {
  ...
  return active.reduce((stats, element) => {
    ...
    if (element.customData?.underscoreSvg) stats.svg += 1;
    if (element.customData?.underscoreLivecode) stats.livecode += 1;
    if (element.customData?.underscoreMediaStream) stats.media += 1;
    return stats;
...
export const underscorePerformanceMonitor = createPerformanceMonitor();
