export default function MediaActorOverlay({ appState, traces = [], strokes = [], markers = [], channelDebug = [] }) {
  ...
  return <svg className="drawerator-media-actor-overlay" width={width} height={height} viewBox={`0 0 ${width} ${height}`} aria-hidden="true">
    ...
    {channelDebug.map(debug => {
      const color = debug.gate?.open ? "var(--drawerator-accent-color, #52d5ff)" : "var(--color-secondary, #8b929e)";
      ...
      return <g key={debug.id} className={`drawerator-brush-channel-debug ${debug.gate?.open ? "is-open" : "is-closed"}`} style={{ color }}>
        {debug.showTrail && debug.trail?.length > 1 && <path className="drawerator-brush-channel-debug-trail" d={pathFor(debug.trail)} />}
        <circle className="drawerator-brush-channel-debug-ring" cx={(debug.point.x + scrollX) * zoom} cy={(debug.point.y + scrollY) * zoom} r={8} />
        {label && <text className="drawerator-brush-channel-debug-label" x={(debug.point.x + scrollX) * zoom + 12} y={(debug.point.y + scrollY) * zoom - 10}>{label}</text>}
      </g>;
    })}
  </svg>;
}
