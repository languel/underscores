import { normalizeUnderscoreObjectRef, underscoreObjectRefKey } from "./underscoreObjectRef.js";
...
export const normalizePhysicsEndpoint = value => {
  ...
  const objectRef = normalizeUnderscoreObjectRef(value.objectRef || value.elementId);
...
export const physicsEndpointKey = value => {
  ...
  const objectKey = underscoreObjectRefKey(endpoint.objectRef);
...
export const normalizePhysicsBody = value => {
  const objectRef = normalizeUnderscoreObjectRef(value?.objectRef);
...
// `underscorePhysics` was the name used by the first canvas-first slice; read
...
export const getPhysicsCustomData = value => {
  const customData = value?.customData && typeof value.customData === "object"
    ? value.customData
    : value;
  return customData?.physics || customData?.underscorePhysics || null;
};
...
export const normalizePhysicsConstraint = value => {
  ...
  return {
    ...
    enabled: value?.enabled !== false,
    kind,
    objectRef: normalizeUnderscoreObjectRef(value?.objectRef),
...
export const removeRelationshipBindingsForElements = (graphValue, elementIds) => {
  ...
  const referencesDeletedElement = objectRef => {
    const reference = normalizeUnderscoreObjectRef(objectRef);
...
export const remapRelationshipGraph = (graphValue, idMap, existingIds = new Set()) => {
  ...
  const remapRef = objectRef => {
    const ref = normalizeUnderscoreObjectRef(objectRef);
    if (!ref) return null;
...
export const findRelationshipOrphans = (graphValue, elements = []) => {
  ...
  const endpointIsOrphaned = endpointValue => {
    ...
    const anchors = element.customData?.underscoreGeometry?.anchors;
...
export class RelationshipWriterRegistry {
  ...
  claim(ownerId, objectRef, channel) {
    const key = `${underscoreObjectRefKey(objectRef)}:${channel}`;
