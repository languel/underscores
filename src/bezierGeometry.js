...
export const hasCubicBezierGeometry = element => isCubicBezierGeometry(element?.customData?.draweratorGeometry);

export const normalizeBezierGeometry = value => ({
  version: Math.max(DRAWERATOR_GEOMETRY_VERSION, Math.round(finite(value?.version, DRAWERATOR_GEOMETRY_VERSION))),
...
const getBezierElementCenter = element => {
  const bounds = getBezierLocalBounds(element.customData.draweratorGeometry);
  return [
...
export const getBezierWorldPath = (element, tolerance = DEFAULT_TOLERANCE) => {
  ...
  return flattenBezierGeometry(element.customData.draweratorGeometry, tolerance / Math.max(1, Math.abs(element.width), Math.abs(element.height)))
...
export const findNearestBezierLocation = (element, worldPoint) => {
  ...
  const detailed = flattenBezierGeometryDetailed(element.customData.draweratorGeometry, 0.002);
  let nearest = null;
...
export const getBezierPathLengthFromAnchors = (worldAnchors, closed = false) => {
  ...
  const element = { id: "length", x: bounds.x, y: bounds.y, width: bounds.width, height: bounds.height, angle: 0, customData: { draweratorGeometry: geometry } };
