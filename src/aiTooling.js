...
/**
 * Parse Underscore command tags without interpreting any other model output.
 ...
 */
export const parseUnderscoreCommandTags = text => {
  const tags = [];
  const expression = /<underscore-command\s+id="([^"]+)"\s*>([\s\S]*?)<\/underscore-command>/gi;
  let match;
...
export const buildAIAutomationGuide = (commands, options = {}) => {
  ...
  return [
    "Underscore automation",
    "Use only <underscore-command id=\"stable.id\">JSON</underscore-command> tags for actions.",
