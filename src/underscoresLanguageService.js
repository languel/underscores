...
export const UNDERSCORES_API = Object.freeze([
  ...
  bridgeApi("__.events", "EventApi", "Inspect, emit, and subscribe to matching event patterns from the shared event stream.", "const latest = __.events.latest(\"media.*\");", "property", 94),
  ...
  bridgeApi("__.events.emit", "emit(name, detail?, metadata?)", "Emit a namespaced event to the shared Underscores event stream. Manim cue controls listen for manim.cue.next, manim.cue.auto, and manim.cue.cue.", "__.events.emit(\"manim.cue.next\", { elementId: __.element.id });", "method", 86),
