...
export const MEDIA_SOURCE_STORAGE_KEY = "underscore_media_source_catalog_v1";
export const MEDIA_ACTORS_ARMED_STORAGE_KEY = "underscore_media_actors_armed_v1";
export const HOLISTIC_SETTINGS_STORAGE_KEY = "underscore_holistic_settings_v1";
...
export const isMediaStreamElement = element =>
  Boolean(element && !element.isDeleted && element.customData?.underscoreMediaStream);
...
export const objectBoundsTargetLabel = element => {
  ...
  return element?.customData?.underscoreLabel || element?.name || element?.type || "Object";
};

export const isMediaSourceElement = element => {
  if (!isMediaStreamElement(element)) return false;
  const kind = normalizeMediaStreamConfig(element.customData.underscoreMediaStream).kind;
...
export const shouldProcessMediaStream = element =>
  ...
  && normalizeMediaStreamConfig(element.customData.underscoreMediaStream).enabled;
