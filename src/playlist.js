...
export const createPlaylistState = (value = {}) => {
  ...
  const items = Array.isArray(source.items) ? source.items.map(item => {
    ...
    trigger: item?.trigger === "event" || item?.trigger === "script" || item?.trigger === "srt" ? item.trigger : "manual",
    };
  }).filter(item => item.elementIds.length) : [];
...
export const createPlaylistItem = ({ elementIds = [], label = "", duration = 5, durationValue = null, transition = "cut", enabled = true, armed = false, trigger = "manual" } = {}) => createPlaylistState({
  defaultDuration: duration,
  defaultDurationValue: durationValue || `${duration} s`,
  items: [{ id: makeId(), elementIds, label, duration, durationValue: durationValue || `${duration} s`, transition, enabled, armed, trigger }],
}).items[0];
