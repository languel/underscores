...
import { migrateLegacyCurveReference } from "./underscoreObjectRef.js";
import { underscoreObjectRefKey, svgNodeObjectRef } from "./underscoreObjectRef.js";
...
const getSvgHostScoreObjects = host => {
  ...
  const rawSvg = host.customData.underscoreSvg;
  ...
  const pathsByNodeId = new Map(getEditableSvgPathNodes(svg.source)
    .filter(path => path.node.underscoreId)
    .map(path => [path.node.underscoreId, path]));
  const objects = Object.entries(svg.metadataMirror?.nodes || {}).flatMap(([nodeId, nodeData]) => {
    ...
    return [{
      ...
      customData: {
        ...
        underscoreObjectRef: ref,
        underscoreSvgHostId: host.id,
      },
    }];
  });
...
export const evaluateScoreFrame = (
  ...
  const resolveCurveElement = data => {
    ...
    if (ref?.kind === "svg-node") return byId.get(underscoreObjectRefKey(ref)) || null;
