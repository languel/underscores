...
import {
  ...
  readSvgUnderscoresMetadata,
  updateSvgNodeData,
  writeSvgUnderscoresMetadata,
...
test("structured editing assigns stable private IDs without replacing authored IDs", () => {
  ...
  assert.match(result.source, /id="layer" data-underscores-id="node-2"/);
  const repeated = ensureSvgNodeIdentities(result.source, { createId: () => "unused" });
  assert.equal(repeated.changed, false);
  assert.equal(repeated.source, result.source);
});

test("embedded Underscores metadata round-trips independently of SVG markup", () => {
  ...
  assert.match(prepared.source, /<metadata data-underscores="v1">/);

  const withData = updateSvgNodeData(prepared.source, "node-2", {
    label: "Score curve",
    iannix: { role: "curve", active: true },
  });
  const metadata = readSvgUnderscoresMetadata(withData);
...
test("metadata writer repairs a self-closing Underscores metadata node", () => {
  const source = `<svg xmlns="http://www.w3.org/2000/svg"><metadata data-underscores="v1"/></svg>`;
  const repaired = writeSvgUnderscoresMetadata(source, {
    nodes: { a: { label: "A & B" } },
  });
  assert.match(repaired, /<metadata data-underscores="v1">\{"version":1,"nodes":\{"a":\{"label":"A &amp; B"\}\}\}<\/metadata>/);
  assert.equal(readSvgUnderscoresMetadata(repaired).nodes.a.label, "A & B");
});
