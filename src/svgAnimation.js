...
const collectSmilAnimations = (document, lanes) => {
  ...
  document.nodes.filter(node => SMIL_ELEMENTS.has(node.localName.toLowerCase())).forEach(node => {
    ...
    const targetId = href.startsWith("#") ? href.slice(1) : parent?.underscoreId || parent?.id || null;
    ...
    lanes.push({
      id: `smil:${node.underscoreId || node.index}`,
      kind: "smil",
      nodeId: targetId,
      animationNodeId: node.underscoreId || null,
...
const collectLooomAnimations = (document, lanes) => {
  ...
  document.nodes.filter(node => (
    ...
    lanes.push({
      id: `looom:${thread.underscoreId || thread.id || thread.index}`,
      kind: "looom",
      nodeId: thread.underscoreId || thread.id || null,
      ...
      keyframes: frames.map((frame, index) => ({
        ...
        nodeId: frame.underscoreId || frame.id || null,
