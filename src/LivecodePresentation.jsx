...
function HtmlPresentation({ element, node, scriptRuntimeRef }) {
  ...
  const token = useMemo(() => `underscore-livecode-${node.nodeId}`, [node.nodeId]);
  const appearance = scriptRuntimeRef.current?.getAppearance?.() || {};
  ...
  useEffect(() => {
    const receive = event => {
      ...
      if (!data?.underscoreLivecode || data.token !== token || data.type !== "ready") return;
      event.source?.postMessage({
        underscoreLivecode: true,
        token,
        type: "bridge",
