...
test("Excalidraw host geometry starts at the element origin even when handles extend past it", () => {
  ...
  const element = { id: "host", ...host.bounds, angle: 0, points: host.points, customData: { draweratorGeometry: host.geometry } };
...
test("legacy bounding-box-based hosts migrate without moving their canonical curve", () => {
  ...
  const element = { id: "legacy", ...legacy.bounds, angle: Math.PI / 8, points: legacyPoints, customData: { draweratorGeometry: legacy.geometry } };
...
test("reframing preserves a rotated curve in world space", () => {
  ...
  const element = { id: "rotated", ...host.bounds, angle: Math.PI / 5, points: host.points, customData: { draweratorGeometry: host.geometry } };
  const geometry = { ...host.geometry, anchors: host.geometry.anchors.map((anchor, index) => index === 0 ? { ...anchor, x: anchor.x - 0.25 } : anchor) };
  const edited = { ...element, customData: { draweratorGeometry: geometry } };
