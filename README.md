...
- **Recordable Sessions & Automation:** `/history` records commands, world-coordinate strokes, coalesced scene edits, score/MIDI events, and optional presentation state. **Loop overdub** turns the active transport loop into a repeating drawing pass: ordinary Excalidraw strokes retain timestamped gesture tracks, redraw at their recorded phase, and layer with later takes. Sessions can be edited, sought, replayed from their captured baseline, exported, or saved as reusable relative/absolute sequences.
...
The shared p5 / Play Core bridge and versioned public `window.__` API are documented in [Underscores Script API](notes/underscores-api.md). Hover or focus the **Script type** control to see the active adapter’s in-app quick reference; p5 and Play Core include this API reference directly in the Info panel.
...
10. Use the **Scene data** section or **Export Underscores .excalidraw** in the main menu to export/import a complete Underscores scene or copy/paste JSON through the clipboard when a browser blocks downloads. Both paths preserve Mods & FX and IanniX custom properties; selection exchange includes hidden runtime cursor hosts and their linked curves, then paste remaps their internal IDs and links. **Copy selection SVG** places vector SVG on the system clipboard; pasting SVG from Underscores, Excalidraw, tldraw, or another vector editor creates native editable lines, freehand paths, rectangles, and ellipses instead of a raster image. Imported paths can then be assigned score roles normally.
...
2. For a repeating drawing pass, set the transport loop, enable **Loop overdub**, then Record (or run `/record loop`). The playhead rewinds to the loop start and begins playing; every completed native or modified stroke joins the loop at the phase where it was drawn.
3. Draw with native or custom brushes and use commands, panels, modifiers, IanniX, or MIDI normally. Strokes preserve scene-coordinate samples, traveled-distance timing, and exact final element snapshots.
4. Stop, edit/mute/reorder steps, seek, choose whether presentation, pointer, and MIDI play back, then Play. Full sessions restore their captured baseline by default.
5. Save one action or a time range as a reusable sequence. Relative insertion remaps IDs and places a fresh copy at the canvas anchor; absolute insertion preserves authored coordinates.
6. Enable Auto-key in the transport to capture object transforms, supported styles, modifier/IanniX properties, and geometry snapshots at global score time.
