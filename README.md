...
3. Click the rendered SVG directly to select its scene host. Command-click or double-click a visible path or compound-path subpath to edit spline-style anchors and handles; Option-drag breaks smooth handle coupling, double-click inserts an anchor, Delete removes one, and Escape exits. Coincident endpoints across compound subpaths behave as one joint by default, including multi-way junctions. Select a shared endpoint and use **Detach joint** in Properties before dragging when that branch should separate deliberately.
4. Open **Properties** for the selected SVG to edit document dimensions, viewBox, the element tree, and attributes. Compound `<path>` data exposes each `M…` subpath as a child without splitting the authored XML node. A selected child can be extracted as a native Drawerator spline, or extracted and assigned Curve, Cursor, or Trigger in one action. In-place edits rewrite only that subpath's commands and preserve its siblings.
...
SVG objects participate in selection, transforms, opacity, naming, Outliner order, history, and scene persistence. The Outliner treats the SVG as one scene layer and exposes its authored XML nodes plus compound-path subpaths as an expandable internal document tree; future general scene grouping remains a separate concern. Direct path editing supports non-transformed line, quadratic, smooth, cubic, closed, and compound path commands. Each supported subpath remains independently editable even when an unsupported sibling is present; arcs and nested SVG transforms remain source-editable. Their live overlay supports declarative SVG styling and SMIL animation. Embedded SVG JavaScript is retained as source for forward compatibility but does not execute in this phase; overlay-aware whole-board export and native/SVG layer interleaving remain compositor work. See [SVG object architecture](notes/svg.md).
