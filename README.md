...
- **Screencast Input:** `/screencast` (Command-Option-I on macOS; Ctrl-Alt-I elsewhere) toggles a tiny draggable overlay that surfaces recent shortcuts, clicks, completed drags, scrolls, and active canvas tools. It deliberately omits high-frequency move samples; History remains the place for complete gesture recording and playback. The active Excalidraw canvas cursor also follows the selected tool (pencil, hand, eraser, or shape) while interactive livecode/model surfaces retain their own pointer behavior. See [Screencast input](notes/screencast-input.md).
