...
5. `drawerator.canvas`, `drawerator.events`, and `drawerator.transport` are the same read-only score bridge available to p5. Source recompiles when it is valid; invalid drafts remain in the editor without replacing the last working frame. Programs run locally and are therefore trusted code.
