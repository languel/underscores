...
- **Semantic Media Streams and Actors:** `/video-input`, `/media-input`, `/holistic`, and `/mapping` cover webcam/media acquisition, adaptively throttled MediaPipe Holistic processing, a shared semantic feature ontology, and persistent position/freedraw bindings. Inputs can remain panel-only or gain optional transformable canvas views; hidden views remain usable upstream. Holistic hosts, display filters, point/line styling, and processing ceilings persist with the scene and seed the next processor. Native grouped landmark snapshots and exact in-place PNG snapshots share the live display choices; trusted p5, Play Core, and Strudel code reads transient frames through `__.streams`. See [Media streams](notes/media-streams.md).
