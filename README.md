...
- **First-Class Livecode Canvas Nodes:** `/live` creates a transparent scene host whose own source, runtime, preview, parameters, and typography persist on the canvas. p5, Play Core, native Strudel, Markdown, LaTeX, sandboxed HTML, and native Orca nodes can run concurrently; the same editor can be docked in Script without duplicating a draft or stopping a node. See [Livecode Nodes](notes/livecode.md) and the [release gate](notes/livecode-licensing.md).
...
3. Programs use the play.core lifecycle: `export const settings`, then optional `boot`, `pre`, `main`, `post`, `pointerMove`, `pointerDown`, and `pointerUp` functions. `main({ x, y, index }, context, cursor, buffer, drawerator)` returns a character or a cell object for each ASCII cell. `context` includes time, frame, cell dimensions, host dimensions, and settings; `cursor` is in cell coordinates and exposes its previous state through `cursor.p`.
4. Declare `// @param threshold = 0.55 (0..1, step: 0.01)` to create a persistent host control. Read it at runtime through `drawerator.params.threshold`. Object parameters use `(object)` and resolve to a live Drawerator canvas object.
...
6. Every Play Core program receives the shared live `drawerator` bridge: host/object identity, `params`, `currentColor`, theme colors, canvas queries, events, transport, and the public API. See the Play Core Info panel for the complete reference.
...
7. `drawerator.canvas`, `drawerator.events`, and `drawerator.transport` are the same read-only score bridge available to p5. Source recompiles when it is valid; invalid drafts remain in the editor without replacing the last working frame. Programs run locally and are therefore trusted code.
