...
- **First-Class Media Streams:** `/video-input`, `/media-input`, and `/holistic` open compact panels for webcam sources, URL/file image/video sources, and MediaPipe Holistic processor objects. Inputs can remain panel-only or gain an optional transformable canvas view; both views and downstream processors consume the same cropped/mirrored output. Animated GIF frames are decoded into that output, while hidden canvas views remain usable upstream. Holistic can hide its source feed, emits normalized landmark events, and snapshots pose/hand geometry into ordinary Drawerator objects. See [Media streams](notes/media-streams.md).
