...
- **Independent Dockable Panels:** AI Assistant, Mods & FX, IanniX, Expressive Synth, Settings, and Console / Info each keep their own visibility, size, and left/float/right placement. Floating panels can coexist; panels docked to the same side become one compact tab group whose active tab shows its icon and label. Transport uses the same interaction for floating or bottom docking. Visibility, active tabs, collapsed docks, and layouts persist independently; toggle panels with `/chat`, `/mods`, `/iannix`, `/synth`, `/settings`, `/console`, and `/transport`.
...
7. **Internal GM Synth** is a lightweight Web Audio fallback when a Web MIDI destination is absent or disconnected. Selecting it, pressing **Play**, or pressing **Enable audio** after reload satisfies the browser's user-gesture requirement. **Test audio** sends a direct C4 diagnostic, **Test Web Audio** isolates the browser's raw audio output, and **Reset audio** disposes and recreates a stuck synth and AudioContext. The compact program section assigns GM programs per melodic channel; channel 10 is percussion. The selected output, fallback policy, program assignments, and History MIDI-armed state are remembered in local storage. Recorded MIDI stays raw and destination-independent, so the active route is chosen at replay time. Physical Web MIDI remains browser-controlled; the Codex embedded harness may deny device enumeration despite a saved site allow entry, so use an external browser for hardware I/O when necessary.
8. **Expressive Synth** is an alternative native Web Audio output for continuous, independently pitched cursor voices. Open `/synth`, choose a subtractive, FM, bowed-string, reed/wind, or pure-tone preset, then map world Y to pitch, stroke width to pressure, and cursor speed to brightness. Each active cursor owns its own oscillator graph, so simultaneous glissandi do not share MIDI pitch bend. No per-voice patching is required: attach one cursor to each curve. **Add & play 6-voice glissando demo** (or `/synth demo`) creates a compact Metastaseis-inspired study, selects the synth, configures a safe bowed preset, and loops it at 1× for 12 seconds. Envelope, filter, damping, vibrato, glide, voice limit, and mapping controls are stored locally and in Drawerator scene metadata; **Test**, **Panic**, and **Reset audio** remain available without a score. See [Expressive Synth architecture and glissando study](notes/expressive-synth.md) for the construction, mapping equations, audio graph, persistence, and verification procedure.
9. Use the **Scene data** section or **Export Drawerator .excalidraw** in the main menu to export/import a complete Drawerator scene or copy/paste JSON through the clipboard when a browser blocks downloads. Both paths preserve Mods & FX and IanniX custom properties; selection exchange includes hidden runtime cursor hosts and their linked curves, then paste remaps their internal IDs and links.
10. **Import trusted .iannix** executes compatible scripts after an explicit trust warning. Native `ask()` declarations automatically become persistent sliders and can be refined with brush-compatible `@param` annotations. This compatibility mode is executable JavaScript, not a security sandbox; unsupported commands are reported.
