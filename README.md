...
- **Semantic Media Streams and Actors:** `/video-input`, `/media-input`, `/holistic`, and `/mapping` cover webcam/media acquisition, MediaPipe Holistic processing, a shared semantic feature ontology, and persistent position/freedraw bindings. Inputs can remain panel-only or gain optional transformable canvas views; hidden views remain usable upstream. Trusted p5, Play Core, and Strudel code reads the same transient frames through `__.streams`, while only processors and binding definitions persist. See [Media streams](notes/media-streams.md).
